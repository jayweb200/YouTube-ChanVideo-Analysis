...
def create_final_report(data, video_analyses, patterns_report, channel_id, top_videos=None):
    ...
    # Save to JSON file
    output_json_file = f"youtube_analysis_results_{channel_id}.json"
    with open(output_json_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    # Save human-readable report to text file
    output_report_file = f"youtube_analysis_report_{channel_id}.md"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Analyze YouTube video data with options for partial execution.')
